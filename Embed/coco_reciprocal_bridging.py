...
def find_reciprocal_graph(G_directed):
    """
    Find a graph that only retains reciprocal edges
    Parameters
    ----------
    G_directed : nx.DiGraph()
    Returns
    -------
    G : nx.Graph()
        The undirected version of G_directed that only retains reciprocal edges and use the average of the weights going both ways 
    """
    G = nx.Graph()
    for u, v, weight in G_directed.edges(data=True):
        if G_directed.has_edge(v, u) and not G.has_edge(u, v):
...
def generate_network_measures(timekey, edges, test_mode):
    ...
    G = find_reciprocal_graph(G_directed)
...
def extract_network_measures(test_mode=False):
    ...
    file_name = 'coco_reciprocal_bridging_test.csv' if test_mode else 'coco_reciprocal_bridging.csv'
